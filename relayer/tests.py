...
def test_parse_body_plain_email():
    ...
    assert samm_id == 3
    assert tx_data.to == '0xD4aF3d17efd18DF0D6a84b8111b9Cd71A039E4a4'
    assert tx_data.value == 1000000000000000
    assert tx_data.data == b'0xd0e30db0'
    assert tx_data.operation == TransactionOperation.call
    assert tx_data.nonce == 23
    assert tx_data.deadline == 1734930048
...
async def test_execution_transaction():
    proofs = [
        ProofStruct(
...
async def test_blockchain_execution_transaction():
    ...
    await blockchain.execute_transaction(
...
if __name__ == '__main__':
    ...
    loop.run_until_complete(test_execution_transaction())
